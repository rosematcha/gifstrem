  ...
  return (
    <div className="min-h-screen bg-charcoal px-6 py-10 text-white">
      <header className="mx-auto flex max-w-5xl items-center justify-between border-b border-slate/30 pb-4">
        ...
        <div className="flex gap-4 text-sm text-coolGray">
          <button
          ...
          <Link className="text-violet hover:text-softViolet font-semibold" to={`/${userData.slug}`} target="_blank">
            Submission link
          </Link>
        </div>
      </header>

      <section className="mx-auto mt-6 grid max-w-7xl gap-6 md:grid-cols-[1fr_auto]">
          ...
          <div className="rounded-card border border-slate/30 bg-graphite p-l shadow-low">
            ...
            <div className="mt-4 space-y-4">
              {(pendingQuery.data ?? []).map((submission) => (
                <div key={submission.id} className="rounded-card border border-slate/30 bg-charcoal p-m">
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div>
                        <p className="font-semibold">{submission.uploaderName}</p>
                      ...
                      <div className="flex gap-2">
                        <button
                          className="rounded-btn border border-coral/40 px-3 py-1 text-sm text-white hover:bg-coral/20"
                          onClick={() => reviewMutation.mutate({ id: submission.id, action: 'deny' })}
                        >
                          Deny
                        </button>
                        <button
                          className="rounded-btn border border-emerald/40 px-3 py-1 text-sm text-white hover:bg-emerald/20"
                ...
                <div key={submission.id} className="rounded-card border border-slate/30 bg-charcoal p-3 text-sm">
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <button
                        className="rounded-btn border border-cyan/40 px-3 py-1 text-xs text-white hover:bg-cyan/20"
                        onClick={() => reviewMutation.mutate({ id: submission.id, action: 'deny' })}
                      >
                        Remove
                      </button>
        ...
        <div className="space-y-6 w-full md:w-[420px]">
          <div className="rounded-card border border-slate/30 bg-graphite p-l shadow-low">
