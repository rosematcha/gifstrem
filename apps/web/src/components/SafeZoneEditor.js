...
export const SafeZoneEditor = ({ resolution, zones, activeIndex, onZoneChange, onSelectZone }) => {
    ...
    const startDrag = (index, handle) => (event) => {
    ...
    return (_jsxs("div", { className: "space-y-3", children: [_jsxs("div", { ref: containerRef, className: "relative w-full max-w-md rounded-lg border border-white/10 bg-slate-900/60", style: {
                    ...
                    touchAction: 'none',
                }, children: [_jsx("div", { className: "absolute inset-0 bg-[radial-gradient(circle,_rgba(148,163,184,0.2)_1px,_transparent_1px)] [background-size:16px_16px]" }), zones.map((zone, index) => {
                        const isActive = index === activeIndex;
                        return (_jsx("div", { className: `absolute select-none rounded-lg border-2 ${isActive
                                ? 'cursor-move border-emerald-400/80 bg-emerald-400/10 shadow-lg shadow-emerald-900/30'
                                : 'border-white/30 bg-white/5'} transition`, style: {
                                left: zone.x * scale,
                                top: zone.y * scale,
                                width: Math.max(zone.width * scale, MIN_WIDTH * scale * 0.5),
                                height: Math.max(zone.height * scale, MIN_HEIGHT * scale * 0.5),
                            }, onPointerDown: startDrag(index, 'move'), children: isActive &&
                                ['tl', 'tr', 'bl', 'br'].map((corner) => (_jsx("span", { onPointerDown: startDrag(index, `resize-${corner}`), className: "absolute h-3 w-3 -translate-x-1/2 -translate-y-1/2 cursor-pointer rounded-full border border-white bg-emerald-300", style: {
                                        left: corner.includes('l') ? 0 : '100%',
                                        top: corner.includes('t') ? 0 : '100%',
                                    } }, corner))) }, `zone-${index}`));
                    })] }), _jsxs("div", { className: "text-xs text-slate-400", children: ["Zone ", activeIndex + 1, ": Position (", Math.round(activeZone.x), ", ", Math.round(activeZone.y), ") px - Size", ' ', Math.round(activeZone.width), " x ", Math.round(activeZone.height), " px"] })] }));
};
